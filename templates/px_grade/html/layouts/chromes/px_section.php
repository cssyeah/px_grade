...
$moduleAttribs          = [];
$moduleAttribs['class'] = $module->position . ' box px-sub-heading-box' . htmlspecialchars($params->get('moduleclass_sfx',''), ENT_QUOTES, 'UTF-8');
$headerTag              = htmlspecialchars($params->get('header_tag', 'h3'), ENT_QUOTES, 'UTF-8');
$headerClass            = htmlspecialchars($params->get('header_class', ''), ENT_QUOTES, 'UTF-8');
$headerAttribs          = [];
$headerAttribs['class'] = $headerClass;

$sub_heading            = $params->get('sub_heading', '');
$mod_desc 							= $params->get('desc','');
...
global $gkModulesCounter;
	...
	if (!empty ($module->content)) {
		...
		if($clear_mode == false) echo '<div class="section-inner"' . (($params->get('backgroundimage')) ? ' style="background-image:url('. $params->get('backgroundimage') . ');" ' : '') . '><div class="container">';

		if($sub_heading) {
			$sub_heading = '<h3 class="sub-heading">'.$sub_heading.'</h3>';
		}

		if($mod_desc) {
			$mod_desc = '<p class="lead">'.$mod_desc.'</p>';
		}

		if($module->showtitle) {
			$title = str_replace('[br]', '<br />', $module->title);
			echo '<div class="section-heading'. $params->get('header_class') .'">
					'.$sub_heading.'
		...
		if($clear_mode == false) echo '<div class="section-body">';
		echo $module->content;
